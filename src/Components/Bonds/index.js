...
function Bonds(props) {
	...
	useEffect(() => {
		tickers.forEach((ticker, i) =>  {
			const lastDate = Object.keys(props.bonds[ticker].data)[Object.keys(props.bonds[ticker].data).length - 1];
			if (+(new Date(lastDate)) < (+(new Date()) - 604800000)) {
				dispatch(autoCompleteStockTickerWithSaga(ticker, new Date(+(new Date(lastDate)) + 604800000), props.bonds[ticker].data[lastDate].quantity, props.bonds[ticker].data[lastDate].currency));
			}
		} )
						...
						<div className={classes.tickerTable}>
							{Object.keys(props.bonds[ticker].data).map((date) =>
								<div key={props.bonds[ticker].data[date].ticker + date} className={classes.tickerDate}>
									...
									<p className={classes.tickerDateCell}>{props.bonds[ticker].data[date].currency}</p>
								</div>
							)}
						</div>
					</div>
