...
import { delTickerDateWithSaga } from '../../store/actions/delTicker';
import { sellTickerWithSaga } from '../../store/actions/sellTicker';
...
function Ticker(props) {
	...
	const delDate = (marketType, ticker, date) => {
		const answer = window.confirm('Are you sure you want to delete this?');
		if (answer) {
			dispatch(delTickerDateWithSaga(marketType, ticker, date));
		};
	};

	const sellTicker = (marketType, ticker, date) => {
		const answer = window.confirm('Are you sure you want to sell this?');
		if (answer) {
			dispatch(sellTickerWithSaga(marketType, ticker, date));
		};
	};
		...
		<div className={classes.tickerTable}>
			...
			{Object.values(props.ticker.data).map((date, i) =>
				<div key={date.ticker + Object.keys(props.ticker.data)[i]} className={classes.tickerDate}>
					...
					<button onClick={() => delDate(props.type, date.ticker, Object.keys(props.ticker.data)[i])}>del</button>
					<button onClick={() => sellTicker(props.type, date.ticker, Object.keys(props.ticker.data)[i])}>sell</button>
				</div>
			)}
