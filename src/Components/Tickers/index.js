				...
				{tickers.map((ticker) =>
					<div key={ticker} className={classes.ticker}>
						<div className={classes.tickerName}>
							<p>{ticker}</p>
							<p>{props.tickers[ticker].data[Object.keys(props.tickers[ticker].data)[Object.keys(props.tickers[ticker].data).length - 1]].description}</p>
						...
						<div className={classes.tickerTable}>
							{Object.keys(props.tickers[ticker].data).map((date) =>
								<div key={props.tickers[ticker].data[date].ticker + date} className={classes.tickerDate}>
									...
									{((props.tickers[ticker].data[date].currency !== 'rur') ? <p className={classes.tickerDateCell}>{props.tickers[ticker].data[date].roubleCost}</p> : '')}
									{((props.tickers[ticker].data[date].currency !== 'rur') ? <p className={classes.tickerDateCell}>{props.tickers[ticker].data[date].roubleTotal}</p> : '')}
								</div>
							)}
						</div>
					</div>
